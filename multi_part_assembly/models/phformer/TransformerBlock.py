...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_masks is not None:
            attention_scores = attention_scores.masked_fill(attention_masks[:, None], float('-inf'))
...
class HybridTransformer(nn.Module):
    def __init__(self, blocks, d_model, num_heads, dropout=None, activation_fn='ReLU', return_attention_scores=False):
        ...
        self.pose_emb = VolumetricPositionEncoding(d_model, voxel_size=0.01, pe_type='sinusoidal')
    ...
    def forward(self, feats, xyz, P_mask):
        ...
        valid_mask = P_mask.unsqueeze(-1).repeat(1, 1, K).reshape(B, -1)
        valid_mask = valid_mask[:, None, :]
        scores_mask = (scores_mask * valid_mask).bool()

        # position encoding
        pe_enc = self.pose_emb(xyz)
        feats0 = feats.permute(0, 2, 1).contiguous()
        attention_scores = []
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0 = self.pose_emb.embed_pos(feats0, pe_enc)
                feats0, scores0 = self.layers[i](feats0, feats0)
            else:
                feats1 = torch.zeros(B, P, K, C).type_as(feats0)
                feats1[P_mask] = feats0

                feats1 = feats1.reshape(B, P*K, C)
...
class FragmentAwareTransformer(nn.Module):
    ...
    def forward(self, feats, xyz, P_mask):
        ...
        valid_mask = P_mask.unsqueeze(-1).repeat(1, 1, K).reshape(B, -1)
        valid_mask = valid_mask[:, None, :]
        scores_mask = valid_mask.bool()
        feats = feats.permute(0, 2, 1).contiguous()
        feats1 = torch.zeros(B, P, K, C).type_as(feats)
        feats1[P_mask] = feats

        onehot = torch.zeros(B, P, K, C).type_as(feats)
        ind = torch.arange(0, P).cuda().long()
        ind = ind[None, :, None, None].repeat(B, 1, K, 1)
        onehot.scatter_(-1, ind, 1)

        feats1 = (feats1 + onehot).reshape(B, P * K, C)
        for i, block in enumerate(self.blocks):
            feats1, scores1 = self.layers[i](feats1, feats1, attention_masks=~scores_mask)
        feats0 = feats1.reshape(B, P, K, C)
        feats0 =feats0[P_mask]
